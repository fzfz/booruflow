...
`schema/` 是数据库、HTTP、错误和数据包结构的机器来源，`config/` 保存运行默认值和发布事实，`bin/` 中的平台脚本组织安装与运维，`scripts/` 保存应用级确定性入口，`tests/` 按 unit、contract、integration 和 e2e 分层。
