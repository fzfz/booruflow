# BooruFlow 文档

[简体中文](README.md) · [English](../en/README.md) · [日本語](../ja/README.md) · [Español](../es/README.md)

BooruFlow 是本机运行的角色、画风、Prompt Tag、模型、LoRA、画师串与 ComfyUI 资源管理工作台。当前应用界面使用中文。

## 用户指南

- [安装](user/installation.md)、[配置](user/configuration.md)、[快速开始](user/quick-start.md)
- [资源关系与维护](user/resources.md)、[日常运行](user/operations.md)
- [更新与恢复](user/update-recovery.md)、[数据迁移](user/data-transfer.md)、[排错](user/troubleshooting.md)

## 社区与开发

- [获取帮助](community/support.md)、[Issue 处理](community/issues.md)、[贡献指南](community/contributing.md)、[PR 规范](community/pull-requests.md)
- [架构](development/architecture.md)、[界面交互规范](development/interface-guidelines.md)、[测试](development/testing.md)、[机器契约](development/contracts.md)、[Catalog/Source 集成](development/catalog-source.md)、[发布](development/releases.md)
- [变更记录](CHANGELOG.md)与仓库根目录 [MIT License](../../LICENSE)
