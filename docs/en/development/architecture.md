...
`schema/` is the machine source for database, HTTP, errors, and packages. `config/` stores runtime defaults and release facts. Platform scripts in `bin/` organize installation and operations; `scripts/` contains deterministic application entry points; `tests/` separates unit, contract, integration, and e2e coverage.
...
[Script directories and maintenance commands](scripts.md)
