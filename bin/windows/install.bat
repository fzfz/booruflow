...
if exist "%BF_DESTINATION%\.git\" if exist "%BF_DESTINATION%\package.json" (call :fail "an existing installation was found; open that directory and run bin\windows\update.bat" & exit /b 1)
...
set "BF_CLONED_INSTALLER_RELATIVE=bin\windows\install.bat"
set "BF_START_RELATIVE=bin\windows\start.bat"
if not exist "%BF_CLONE_DESTINATION%\%BF_CLONED_INSTALLER_RELATIVE%" (
  echo Selected release clone remains at: "%BF_CLONE_DESTINATION%" 1>&2
  call :fail "the selected release does not use the current Windows bin layout; remove the displayed clone directory, then download the selected tag's published install.bat attachment and run it with another empty installation directory"
  exit /b 1
)
