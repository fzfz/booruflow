...
import { TEST_PORTS, startTestApp } from '../../../scripts/testing/start-test-app.mjs';
...
test('隔离测试应用在没有模型服务配置时使用 fake 语义适配器启动', async () => {
  const app = await startTestApp({ testMode: true });
  try {
    assert.equal(app.baseUrl, `http://127.0.0.1:${TEST_PORTS.public}`);
    assert.equal(app.internalBaseUrl, `http://127.0.0.1:${TEST_PORTS.internal}`);
...
test('统一环境关闭时退出子进程并删除临时根，下一次运行使用新的隔离根', async () => {
  const first = await startTestApp({ testMode: true });
  const firstRoot = first.root;
  await first.close();
  await assert.rejects(access(firstRoot), /ENOENT/u);

  const second = await startTestApp({ testMode: true });
  try {
...
test('迁移、导入和启动失败向调用方传播，测试不会静默改用假服务', async () => {
  for (const [stage, options] of [
    ...
    try {
      await assert.rejects(
        async () => { app = await startTestApp({ testMode: true, ...options }); },
        new RegExp(stage, 'iu')
      );
    } finally {
      await closeIfStarted(app);
