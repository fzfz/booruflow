...
if [ -e "$BF_DESTINATION" ]; then
  ...
  if [ -d "$BF_DESTINATION/.git" ] && [ -f "$BF_DESTINATION/package.json" ]; then
    bf_install_fail "an existing installation was found; run $BF_DESTINATION/update.sh"
  fi
...
if [ "$BF_INSTALL_IN_PLACE" -eq 1 ]; then
  cmp -s "$BF_INSTALLER_PATH" "$BF_CLONE_DESTINATION/install.sh" || bf_install_fail "the installer differs from $BF_TAG; download that tag's install.sh or choose another empty installation directory"
  if [ -x "$BF_CLONE_DESTINATION/install.sh" ]; then chmod +x "$BF_INSTALLER_PATH"; else chmod -x "$BF_INSTALLER_PATH"; fi
  for BF_CLONED_ENTRY in "$BF_CLONE_DESTINATION"/.[!.]* "$BF_CLONE_DESTINATION"/..?* "$BF_CLONE_DESTINATION"/*; do
    [ -e "$BF_CLONED_ENTRY" ] || continue
    [ "$(basename -- "$BF_CLONED_ENTRY")" = 'install.sh' ] && continue
    mv "$BF_CLONED_ENTRY" "$BF_DESTINATION/" || bf_install_fail 'the temporary clone could not be moved into the installation directory'
  done
  rm -f "$BF_CLONE_DESTINATION/install.sh"
